...
async def async_setup_entry(
    ...
    # NOTE: Currently disabled due to Jellyfin API limitations
    # Jellyfin's Storage API returns filesystem-level data, not per-library sizes
    # Multiple libraries on the same disk would show identical values
    # This would be misleading for users
    # 
    # Potential solutions for future implementation:
    # 1. Sum MediaSources.Size for all items in each library (performance intensive)
    # 2. Wait for Jellyfin to add per-library size calculation to their API
    #
...
# NOTE: JellyHALibraryStorageSensor is currently disabled due to Jellyfin API limitations
# The Jellyfin /System/Info/Storage endpoint returns filesystem-level usage data,
# not actual per-library storage consumption. Multiple libraries on the same disk
# would report identical (and misleading) filesystem usage values.
#
# To properly implement this feature, we would need to:
# 1. Query all items in each library individually with MediaSources field
# 2. Sum the Size field from MediaSources for each item
# 3. This would require additional API calls per library and increase load time
#
# Future implementation should wait for Jellyfin to add native per-library
# size calculation to their API, or implement the MediaSources aggregation
# with proper caching and performance considerations.
